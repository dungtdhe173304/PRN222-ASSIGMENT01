...
using ThieuDucDungMVC.Services;

namespace ThieuDucDungMVC.Controllers
{
    ...
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> CreateNewsArticle(NewsArticle model)
        {
            if (ModelState.IsValid)
            {
                ...
                string adminEmail = "dungtdhe173304@fpt.edu.vn"; 
                string subject = "🆕 New News Article Pending Approval";
                string articleLink = Url.Action("ViewNews", "Home", new { id = model.NewsArticleId }, Request.Scheme);
                string message = $@"
            <h3>A new article has been created and is awaiting approval:</h3>
            <p><strong>Title:</strong> {model.NewsTitle}</p>
            <p><strong>Author ID:</strong> {model.CreatedById}</p>
            <p><strong>View Article:</strong> <a href='{articleLink}'>Click here</a></p>
        ";

                // ✅ Gửi email bằng MailKit
                await _emailService.SendEmailAsync(adminEmail, subject, message);

                TempData["Success"] = "✅ News article added successfully! Admin has been notified.";
